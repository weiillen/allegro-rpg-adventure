...
These limitations are documented rather than silently corrected so the repository remains a faithful record of the submitted implementation.

## Academic and authorship context

This project was created in an introductory programming course environment and appears to build on a provided Allegro game framework. The archive does not include commit history or a line-by-line attribution record, so this portfolio describes the observable completed system without claiming that every framework component originated from one author.

## Asset and licensing notice

The archive did not include a complete asset-attribution or licensing file. Images, audio, and `Minecraft.ttf` are preserved exactly as submitted so the project remains reproducible, but their redistribution rights should be confirmed before making the repository public. See [`docs/ASSET_AND_LICENSE_NOTES.md`](docs/ASSET_AND_LICENSE_NOTES.md).

No open-source license is added to this curated repository because the ownership and licensing of all framework and asset components cannot be established from the uploaded archive alone.

## Source integrity

[`ORIGINAL_FILE_MANIFEST.tsv`](ORIGINAL_FILE_MANIFEST.tsv) records the original archive path, repository path, byte size, and SHA-256 digest for every preserved artifact. See [`docs/PRESERVATION.md`](docs/PRESERVATION.md) for the exact curation boundary.
